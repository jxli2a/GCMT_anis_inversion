...
im_alpha = 1 - isnan(im) ; % transparency data 
...
h = surface('XData',x,'YData',y,'ZData',z,...
    'CData', im,...
    'FaceColor','texturemap',...
    'AlphaData',im_alpha,'FaceAlpha','texturemap','AlphaDataMapping','none',...  % for make the nan points transparency % By Hao Hu
    'EdgeColor','none',...
    'LineStyle','none',...
    'Marker','none',...
    'MarkerFaceColor','none',...
  ...
  % Reuse old surface
  set(handle,'XData',x,'YData',y,'ZData',z,'CData',im,'AlphaData',im_alpha);
  h = handle;
end
